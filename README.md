...
* `autoLogging.ignore`: set to a `function (req, res) => { /* returns boolean */ }`. Useful for defining logic based on req and res properties (such as a user-agent header or status code) to ignore requests. It is consulted for every automatic log of a request, which means both the "request received" log (when `customReceivedMessage` or `customReceivedObject` is set) and the "request completed" / "request errored" log. Note that it is called once per log, so when it runs for the "request received" log the response has not been written yet and `res.statusCode` still holds its default value.
